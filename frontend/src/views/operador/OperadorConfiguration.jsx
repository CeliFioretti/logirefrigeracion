...
const OperadorConfiguration = () => {
    ...
    // Estado para las alertas (mantener deshabilitado por ahora)
    ...
    }, [usuario]); // Se ejecuta cada vez que el objeto 'usuario' cambia

    // Manejador para el envío del formulario de actualización de perfil
    const handleProfileSubmit = async (e) => {
        e.preventDefault(); // Prevenir el comportamiento por defecto del formulario
        setLoadingProfile(true); // Activar el estado de carga
        setErrorProfile(null); // Limpiar errores anteriores
        setSuccessMessageProfile(null); // Limpiar mensajes de éxito anteriores
        ...
        try {
            // Realizar la solicitud PUT a la API para actualizar el perfil
            const response = await axiosInstance.put('/usuarios/configuracion', {
                nombre: nombre.trim(),
                correo: correo.trim(),
            }, {
                headers: {
                    Authorization: `Bearer ${usuario.token}` // Enviar el token de autenticación
                }
            });

            setSuccessMessageProfile(response.data.message); // Mostrar mensaje de éxito
            // Actualizar el contexto del usuario con los nuevos datos recibidos del backend
            setUsuario(prev => ({
                ...prev,
            ...
            setLoadingProfile(false); // Desactivar el estado de carga
        }
    };
    ...
    const handlePasswordChange = async (e) => {
        ...
        try {
            // Realizar la solicitud PUT a la API para cambiar la contraseña
    ...
    // Si el objeto usuario no está disponible (ej. no logueado o cargando), muestra un spinner
    if (!usuario) {
        return (
    ...
    return (
        <Box sx={{ p: 3, maxWidth: 800, mx: 'auto' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 4 }}>
                ...
                <Typography variant="h5" component="h1" gutterBottom align="center" sx={{ flexGrow: 1 }}>
                    Mi Configuración
                </Typography>
            </Box>

            {/* Sección de Alertas (deshabilitada por ahora) */}
            <Paper elevation={3} sx={{ p: 4, mt: 3, mb: 4, borderRadius: 2 }}>
...
export default OperadorConfiguration;